...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: " + sys.argv[0] + " chrome [prefix] path [Optional] ")
        print("===Support list===")
        print(
            "sogou --- sogou browser\r\n360 --- 360 safe browser\r\n360cse --- 360 cse browser\r\n2345 --- 2345 browser\r\nqq --- QQ browser\r\nchrome --- chrome browser [ <80.x ]\r\nfirefox --- firefox browser [need prefix]\r\nothers --- Other Browser based on chromium open source project [ <80.x ]")
        sys.exit()
    bro = sys.argv[1]
    if bro == "sogou":
        ...
        path = sys.argv[2]
        cpath = os.environ['LOCALAPPDATA'] + r"%s"%path
    elif bro == "firefox":
        if len(sys.argv) < 3:
            print("Miss prefix !")
            ...
            if len(sys.argv) == 3:
                cpath = os.environ[
                            'APPDATA'] + r"\Mozilla\Firefox\Profiles\\" + prefix + r".default-release\cookies.sqlite"
            else:
                cpath = os.environ['APPDATA'] + r"\Mozilla\Firefox\Profiles\\" + prefix + r".default-" + sys.argv[
                    3] + r"\cookies.sqlite"
    else:
